...
import com.verisign.iot.discovery.utils.*;
...
public class DnsServicesDiscovery extends Configurable implements DnsDiscovery {
	...
	@Override
	public Set<String> listServiceTypes ( Fqdn browsingDomain, boolean secValidation ) throws LookupException,
	...
	private class ServicesLookupHelper {
			...
			do {
				...
				try {
					...
					statusChange( StatusChangeEvent.build( browsingDomain.fqdnWithPrefix( label ),
							Type.string( Type.PTR ), StatusChangeEvent.castedList( set.getTexts() ) ) );
				}
				catch ( LookupException le ) {
					if ( le.dnsError().equals( StatusCode.SERVER_ERROR )
							|| le.dnsError().equals( StatusCode.RESOURCE_INSECURE_ERROR ) ) {
		...
		private Record[] lookup ( LookupContext ctx ) throws LookupException {
			Lookup lookup
					= DnsUtil.instantiateLookup( ctx.getDomainName().fqdnWithPrefix( ctx.getPrefix() ),
					ctx.getResolver(), ctx.getRrType(),
			...
			else if ( outcome.equals( StatusCode.NETWORK_ERROR ) ) {
				errorsTrace.get().put(
		...
		public Set<String> getServiceTypeNamesFromRecords ( Record[] records, LookupContext lookupContext ) throws LookupException {
